...
print('Компьютер выбирает любое слово и переставляет в нём буквы.Ваша задача восстановить исходные слова')
print('За каждую подсказку программа списывает 6 баллов')
COUNT_INSIDE = 3

WORDS = ('программирование', 'зачёт', 'метро', 'машина', 'шоколад')
...
print('Угадайте анаграмму, пожалуйста:'  ,jumble)
...
while correct != user_word and count_penalty < COUNT_INSIDE :
    ...
    if correct != user_word :
        print('Неправильно')
        inside = input('Воспользоваться подсказкой? (да/нет)')
        if inside.lower() == 'да' :
            print ('Подсказка', insides[count_penalty])
            count_penalty += 1
            print ('У Вас отняли', penalty,'баллов!')
            ball -= penalty
